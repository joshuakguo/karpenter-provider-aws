...
func (in *EC2NodeClass) PlacementGroup() []PlacementGroup {
	return in.Status.PlacementGroup
}
...
func PlacementGroupStateFromEC2(state ec2types.PlacementGroupState) (PlacementGroupState, error) {
	if state == "" {
		return PlacementGroupStateAvailable, nil
	}
	...
	if !ok {
		return "", serrors.Wrap(
			fmt.Errorf("received placement group with state from ec2"),
			"state", string(state),
		)
	}
	return resolvedType, nil
...
func PlacementGroupStrategyFromEC2(strategy ec2types.PlacementStrategy) (PlacementGroupStrategy, error) {
	...
	if !ok {
		return "", serrors.Wrap(
			fmt.Errorf("received placement group with strategy from ec2"),
...
func PlacementGroupFromEC2(pg *ec2types.PlacementGroup) (PlacementGroup, error) {
	...
	// PartitionCount is only set for partition placement groups
