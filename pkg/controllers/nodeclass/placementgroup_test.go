...
var _ = Describe("NodeClass Placement Group Reconciler", func() {
	...
	It("should set condition true and clear status when no placement group selector is specified", func() {
	...
	It("should resolve a cluster placement group by name", func() {
		...
		nodeClass = ExpectExists(ctx, env.Client, nodeClass)
		Expect(nodeClass.StatusConditions().Get(v1.ConditionTypePlacementGroupReady).IsTrue()).To(BeTrue())
		Expect(nodeClass.Status.PlacementGroup).To(HaveLen(1))
		Expect(nodeClass.Status.PlacementGroup[0]).To(Equal(v1.PlacementGroup{
			ID:       "pg-cluster123",
			Name:     "my-cluster-pg",
	...
	It("should resolve a placement group by ID", func() {
		...
		Expect(nodeClass.StatusConditions().Get(v1.ConditionTypePlacementGroupReady).IsTrue()).To(BeTrue())
		Expect(nodeClass.Status.PlacementGroup).To(HaveLen(1))
		Expect(nodeClass.Status.PlacementGroup[0].ID).To(Equal("pg-cluster123"))
		Expect(nodeClass.Status.PlacementGroup[0].Name).To(Equal("my-cluster-pg"))
	})
	...
	It("should clear status when placement group selector is removed", func() {
	...
	It("should update status when placement group selector changes", func() {
		...
		nodeClass = ExpectExists(ctx, env.Client, nodeClass)
		Expect(nodeClass.Status.PlacementGroup).To(HaveLen(1))
		Expect(nodeClass.Status.PlacementGroup[0].ID).To(Equal("pg-spread789"))
		Expect(nodeClass.Status.PlacementGroup[0].Strategy).To(Equal(v1.PlacementGroupStrategySpread))
	})
})
